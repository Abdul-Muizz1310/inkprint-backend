...
from inkprint.platform import platform_token
...
def _make_app(*, demo_mode: bool) -> TestClient:
    ...
    @app.get("/public-key.pem", include_in_schema=False)
    async def _public_key() -> dict[str, str]:
        return {"pem": "stub"}
...
def _token(priv_pem: str, *, expired: bool = False) -> str:
    ...
    return jwt.encode(
        {"sub": "bastion", "role": "admin", "service": "inkprint", "exp": exp},
        priv_pem,
        algorithm="EdDSA",
    )
...
def test_non_demo_with_key_rejects_missing_token(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert client.get("/public-key.pem").status_code == 200
